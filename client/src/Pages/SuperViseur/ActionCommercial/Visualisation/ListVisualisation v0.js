...
function ListVisualisation() {
  ...
  const getSegment = useCallback(
    async (list) => {
      ...
      if (idRole === 2) {
        if (test === false)
          res = await dispatch(
            getClientSegmentById({ idSeg, idRole, idBricks, dd, df, id,idUser })
          );
        else
          res = await dispatch(
            getAllClientSeg({ idSeg, idRole, idBricks, dd, df, id,idUser })
          );
      } else res = await dispatch(getCommandeByEtat({ id, idUser }));
  ...
  const pop_up = useCallback(async (id) => {
  ...
  function GroupPacks({groupPacks}){
    ...
    groupPacks.map((val,key)=>{
      var res = val.split("@");
      span.push(<span className="pack_action" key={"pack"+key} onClick={()=>{pop_up(res[1])}}>{res[0]}, </span>)
    })
    return(span)
  }
  return (
    <>
      ...
      <Container fluid>
        ...
        <Row>
                            ...
                            onClick={(e) => {
                              navigate(
                                "/detailVisualisation/" +
                                  id +
                                  "/" +
                                  cell.row.original.id_pharmacie
                              );
                              localStorage.setItem(
                                "returnVis",
                                `/visualisation/${id}/${location.idLine}`
          ...
          {idRole === 1 || idRole === 2 ?
            <Col md="12">
              <h4 className="title">Liste des pharmacies retirées</h4>
              <Card className="table-visualisation-action">
                <Card.Body>
                  <ReactTable
                    data={commandeR}
                        ...
                        Cell: ({ cell }) => (
                          <div>
                            <Button
                              id={"idLigneV_" + cell.row.original.pharmacies.id}
                              onClick={(e) => {
                                navigate("/detailVisualisation/" + id + "/" + cell.row.original.id_pharmacie);
                                localStorage.setItem("returnVis",`/visualisation/${id}/${location.idLine}`);
                              }}
                              className="btn btn-info"
                            >
                              Détail
                        ...
                        Cell: ({ cell }) => (
                          <div className="block-action">
                            <Button
                              ...
                              onClick={(e) => {
                                localStorage.setItem("returnVis",`/visualisation/${id}/${location.idLine}`);
                                navigate(`/detailSuivi/${cell.row.original.id_pharmacie}/${id}`);
                              }}
                              variant="success"
                              size="sm"
                            >
